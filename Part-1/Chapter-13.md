...
Manually typing the same 10 commands every time you deploy an app is slow and error-prone. 

A bash script turns that into one command: `./deploy.sh`. 

This is literally the foundation that CI/CD pipelines (Jenkins, GitHub Actions) are built on — Part 3 and 4 of this book will show you that even "advanced" CI/CD pipelines are, under the hood, sequences of bash commands.
...
#!/bin/bash
```
> This is a comment. The line above is called a "shebang" — it tells the OS which interpreter to use to run this script.
```bash
...
```bash
chmod +x deploy.sh      # Make it executable (see Chapter 4)
...
|$PATH |Colon-separated list of directories bash searches for executable commands |
...
1. What is a shebang line, and why is it necessary?
2. What does set -e do, and why is it important in production scripts?
3. What is the $PATH environment variable, and how does bash use it?
4. How do you make an environment variable persist across reboots/logins?
5. Write a bash script that checks if a service is running and restarts it if not.
6. What's the difference between $@ and $# in a bash script?
7. Why is quoting variables ("$VAR") considered a best practice?
