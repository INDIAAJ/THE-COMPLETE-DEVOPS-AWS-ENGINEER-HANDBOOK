...
-rwxr-xr--  1 ambika devops  1024  Jul 1 10:00  deploy.sh
...
Type: - → regular file
Owner (ambika): rwx → read, write, execute (4+2+1 = 7)
...
bashchmod 755 deploy.sh     # rwxr-xr-x — owner full, group/others read+execute
...
bashchmod u+x script.sh      # Add execute permission for owner (u=user/owner)
...
bashchown ambika file.txt              # Change owner to 'ambika'
...
bashchgrp devops file.txt   # Equivalent to: chown :devops file.txt

4.8 Special Permissions (Advanced)

Special PermissionNumericSymbolPurposeSUID (Set User ID)4000s in owner's execute slotProgram runs with the file owner's privileges, not the person running it (e.g., /usr/bin/passwd runs as root even when a normal user runs it, so it can update /etc/shadow)SGID (Set Group ID)2000s in group's execute slotFiles created inside a directory inherit the directory's groupSticky Bit1000t in others' execute slotIn a shared directory (like /tmp), users can only delete their OWN files, even if others have write access

bashchmod 4755 /usr/bin/somebinary   # Set SUID
...
What does permission 755 mean in symbolic form?
Why does SSH refuse to connect if your private key has permission 644?
What is the SUID bit, and can you give a real example of where Linux uses it?
Why is chmod 777 considered dangerous?
What permission does a directory need for a user to cd into it?
How would you recursively give a web server user ownership of a directory?
